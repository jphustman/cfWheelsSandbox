...
(function ( $ ){
	...
	$.fn.extend({
		...
		callRemote: function () {
			...
			url     = el.attr('action') || el.attr('href') || el.attr('data-this-action'),
	...
	$('form[data-remote]').on( 'submit.wheels', function (e) {
		$(this).callRemote();
		e.preventDefault();
	});
	...
	$('a[data-remote],input[data-remote]').on('click.wheels', function (e) {
		$(this).callRemote();
		e.preventDefault();
	});
	...
	var disable_with_input_function = function () {
		$(this).find(disable_with_input_selector).each(function () {
			var input = $(this);
			var enable_label = (input.data('disable-with') === true) ?
	...
	$(disable_with_form_remote_selector).on('ajax:beforeSend.wheels', disable_with_input_function);
	$(disable_with_form_not_remote_selector).on('submit.wheels', disable_with_input_function);

	$(disable_with_form_remote_selector).on('ajax:complete.wheels', function () {
		$(this).find(disable_with_input_selector).each(function () {
	...
	/*if (!( (jqueryVersion === '1.4.3') || (jqueryVersion === '1.4.4'))){
	  alert('This wheels.js does not support the jQuery version you are ' +
	  'using. Please read documentation for more information.');
	  }*/
